...
socket.on('state', (players, bullets, walls) => {
    ...
    Object.values(players).forEach((player) => {
        ...
        if(player.socketId === socket.id){
            // Your player
			camera.position.set(
			    player.x + player.width/2 - 0 * Math.cos(player.angle),
			    80,
                player.y + player.height/2 - 0 * Math.sin(player.angle)
            );
            ...
            context.fillText(' Points: ' + player.point, 13, 40);
            context.fillText('Health: ' + player.health, 20, 75);
        }
    });
